...
namespace AutoRepair.Util
{
    public static class Helper
    {
        ...
        public static PluginInfo GetBundledMod(string name)
        {
            ...
            {
                Debug.Log($"[{Mod.name}] Helper.GetBundledMod({name}) ERROR");
                Debug.LogException(e);
            }
            return null;
        }

        // set enabled state of bundled mod
        public static void SetBundledModState(string name, bool enabled)
        {
            try
            {
                GetBundledMod(name).isEnabled = enabled;
            }
            catch (Exception e)
            {
                Debug.Log($"[{Mod.name}] Helper.SetBundledModState({name},{enabled}) ERROR");
                Debug.LogException(e);
            }
        }
        ...
        */
        ...
        public static bool AnyInstalled(HashSet<ulong> list)
        {
            try
            {
                foreach (PluginInfo plugin in Singleton<PluginManager>.instance.GetPluginsInfo())
                {
                    if (!plugin.isBuiltin && list.Contains(plugin.publishedFileID.AsUInt64))
                    {
                        return true;
                    }
                }
            ...
            {
                Debug.Log($"[{Mod.name}] Helper.AnyInstalled() ERROR");
                Debug.LogException(e);
            }
            return false;
        }
        ...
        public static bool AnyEnabled(HashSet<ulong> list)
        {
            try
            {
                foreach (PluginInfo plugin in Singleton<PluginManager>.instance.GetPluginsInfo())
                {
                    if (!plugin.isBuiltin && list.Contains(plugin.publishedFileID.AsUInt64) && plugin.isEnabled)
                    {
                        return true;
                    }
                }
            ...
            {
                Debug.Log($"[{Mod.name}] Helper.AnyEnabled() ERROR");
                Debug.LogException(e);
            }
            return false;
        }
